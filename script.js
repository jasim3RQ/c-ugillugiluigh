...
function loadOrders() {
    ...
    db.ref('orders').orderByChild('user').equalTo(userPhone).on('value', snap => {
        ...
        // ترتيب الطلبات بحيث يكون الأحدث في الأعلى بناءً على الوقت (timestamp)
                ...
                <div class="order-card" onclick='openDetails(${JSON.stringify(o)})' style="background:white; border-radius:15px; margin-bottom:15px; width:100%; border:1px solid #ddd; overflow:hidden; cursor:pointer; flex-shrink: 0; ${cardStyle}">
                    <div style="padding:20px; display:flex; justify-content:space-between; align-items:center;">
                        <div>
                            ...
                            <small style="color:#888;">${currentLang === 'ar' ? 'إضغط لعرض التفاصيل الكاملة' : 'Click for full details'}</small>
                        </div>
                        <div style="text-align:left;">
    ...
    if (o.isBundle && o.deliveries) {
        o.deliveries.forEach((del, idx) => {
            ...
            <div class="info-card-huge" style="padding:15px; border-radius:12px; margin-bottom:15px; border-right:5px solid #555; background:#f9f9f9;">
                ...
                <p style="margin:8px 0;"><b>${t('منطقة التسليم...')}</b> ${del.dArea}</p>
                <p style="margin:5px 0;"><b>${t('مجمع')}:</b> ${del.dBlock || '-'} | <b>${t('طريق')}:</b> ${del.dRoad || '-'} | <b>No:</b> ${del.dHouse || '-'}</p>
                ${del.dType ? `<p style="margin:5px 0;"><b>Apt/Floor:</b> ${del.dType}</p>` : ''}
                <p style="margin:5px 0;">📞 ${t('رقم هاتف الزبون')}: <span dir="ltr">${del.dPhone}</span></p>
        ...
        <div class="info-card-huge" style="padding:15px; border-radius:12px; margin-bottom:15px; border-right:5px solid #555; background:#f9f9f9;">
            ...
            <p style="margin:8px 0;"><b>${t('منطقة التسليم...')}</b> ${o.dArea}</p>
            <p style="margin:5px 0;"><b>${t('مجمع')}:</b> ${o.dBlock || '-'} | <b>${t('طريق')}:</b> ${o.dRoad || '-'} | <b>No:</b> ${o.dHouse || '-'}</p>
            ${o.dType ? `<p style="margin:5px 0;"><b>Apt/Floor:</b> ${o.dType}</p>` : ''}
            <p style="margin:5px 0;">📞 ${t('رقم هاتف الزبون')}: <span dir="ltr">${o.dPhone}</span></p>
    ...
    if(detailsArea) {
        ...
        detailsArea.innerHTML = `
            ...
            <div class="info-card-huge" style="padding:15px; border-radius:12px; margin-bottom:15px; border-right:5px solid var(--red); background:#fff9f9;">
                ...
                <p style="margin:8px 0;"><b>${t('منطقة الاستلام...')}</b> ${o.pArea}</p>
                <p style="margin:5px 0;"><b>${t('مجمع')}:</b> ${o.pBlock || '-'} | <b>${t('طريق')}:</b> ${o.pRoad || '-'} | <b>No:</b> ${o.pHouse || '-'}</p>
                ${o.pType ? `<p style="margin:5px 0;"><b>Apt/Floor:</b> ${o.pType}</p>` : ''}
                <p style="margin:5px 0;">📞 ${t('رقم هاتف المستلم منه')}: <span dir="ltr">${o.pPhone}</span></p>
...
// 🚀 تم إزالة target من الدالة لمنع أي تعليق أو تجميد للشاشة
